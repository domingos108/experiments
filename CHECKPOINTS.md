...
**Última atualização:** 2026-07-21, ao final da Tarefa 3.9.
...
| 3.9 | Corrigir baseline desatualizado (`1amv1`) + limpar `selector__k` morto | ✅ Preparado, **execução manual pendente** (ver Seção 3) | `.pkl` `identity` arquivado; `arima_mlp.ipynb`/`arima_mlp_rf_embedded.ipynb`/`arima_mlp_lasso.ipynb` prontos com `force=True`, nenhum executado ainda |
...
1c. **Nova divergência descoberta na Tarefa 3.9 (pré-check), NÃO corrigida:**
   `data/result/chamados/*_1mlp.pkl` (baseline MLP single, `mlp_exec.ipynb`) ainda tem
   `diff_kpss=True` persistido, mas o código-fonte usa `diff_kpss=False` desde o mesmo commit
   `43dae50` que corrigiu o `1amv1` — e essa mudança já está documentada como correta no
   CLAUDE.md Seção 3.5 há tempos, sem o `.pkl` nunca ter sido regenerado. Fora do escopo da
   Tarefa 3.9 (que tratou só `1amv1`) — decisão de regenerar `1mlp` fica para uma tarefa
