...
**Nota de proveniência (2026-08-29) — seed opt-in `estimator_random_state_base`:** `GridSearch` (e `grid_seach_multiple_bases`) ganhou o parâmetro aditivo `estimator_random_state_base` (default `None` = comportamento byte-a-byte inalterado). Quando setado (**convenção do projeto: `42`, constante fixa, não exposta por notebook**), a i-ésima das `model_exec` repetições recebe `random_state = 42 + i` **apenas no estimador** (`estimator__random_state` no `Pipeline` de FS — o step **precisa** se chamar `estimator`, senão falha alto; `random_state` no estimador nu dos baselines). **Isto NÃO substitui a decisão de estocasticidade acima**: as 10 repetições continuam sendo 10 inicializações distintas e a variância entre elas continua sendo o dado de interesse — a seed só a torna **reprodutível entre execuções**.

**Alcance real da reprodutibilidade (importante):** o seletor **nunca** é semeado (`mutual_info`/`rf_embedded`/`rfecv` mantêm `random_state=None`, PLANO_ARQUITETURA.md 1.5). Logo, com a seed ativa, a reprodução bit-a-bit entre execuções vale **só para as estratégias de seletor determinístico** — `f_test` e `lasso` (`LassoCV` `selection='cyclic'` + `TimeSeriesSplit`). Para `mutual_info`/`rf_embedded`/`rfecv`, a seed fixa o componente de inicialização da MLP (a maior fonte de variância — foi o que fez `airlines` ir de 19→22), mas o seletor ainda escolhe features diferentes a cada execução → o RMSE final ainda varia (variância residual, menor, e intencional por §1.5). SVR (`svr/*`, `arima_svr/*`) é determinístico e não usa o parâmetro.

Motivação: os notebooks de FS chamam `GridSearch(...).execution()` num laço direto, e re-executar um notebook (ex. ao adicionar uma série a `fs_series_list`) re-sorteava as séries já validadas. O laço de `execution()` também passou a honrar `force=False`/`file_exists` (com `.pkl` não-vazio; antes só `grid_seach_multiple_bases` fazia) — fecha a via do re-sorteio **acidental**. A partir de então as famílias MLP (`arima_mlp/*`, `mlp/*`) rodam com `estimator_random_state_base=42`.
