...
def open_format_train_val_test(base_name, normalize, lag_size):

    df = load_raw_data(base_name)
    ts_univariate = df['y'].values

    test_size = int(config.TEST_SIZE * ts_univariate.shape[0])

    val_size = int(config.VAL_SIZE * ts_univariate.shape[0])
    ...
    df_val = df_train = df_windowed[-(test_size+val_size): -test_size]
    df_test = df_windowed[-test_size:]

    return df_train, df_val, df_test, min_max_scaler