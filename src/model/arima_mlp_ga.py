...
def start_population(
    ...
    mlp = MLPRegressor(hidden_layer_sizes=(hidden_layer_sizes, ), max_iter=1)
    mlp = mlp.fit(X, y)
...
class MetaHeuristicMLP(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        best_fitness = []
        best_individuals = []
        for geracao in range(self.iterations):

            metrics = metric_models(mlps, self.X, self.y)
            best_arg = np.argmin(metrics)

            best_fitness.append(metrics[best_arg])
        ...
        self.selected_individual = best_individuals[np.argmin(best_fitness)]
    ...
    def predict(self, X):
        ...
        return self.final_mlp.predict(X)